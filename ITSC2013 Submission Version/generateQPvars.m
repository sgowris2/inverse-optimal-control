function [A,b,Aeq,beq] = generateQPvars(tf,alpha,t0,l0,deltaMin,deltaMax,iMax,n) % you will need 
%to modify as needed (possibly output f too)

%the inputs are:
%tf is a scalar final time
%t0 is a scalar initial time
...
A=zeros(6*n,iMax*n + 2*n); %preallocate
Aeq=zeros(n+1,iMax*n + 2*n); %preallocate
b=zeros(6*n,1); %preallocate
...
for k = 2:n
    ...
    Aeq(eqNum,t(k)) = -1;
    Aeq(eqNum,t(k-1)) = 1;
    beq(eqNum) = 0; %not really needed, b was initialized as zeros
    eqNum = eqNum + 1;
end
...
k = 0;
    for i = 1:iMax
        ...
        A(ineqNum,l(i,k+1)) = -1;
        b(ineqNum) = -l0(i); %this line handles the initial condition on led b as zeros
        ineqNum = ineqNum + 1; 
    end
...
for k = 1:n-1
    for i = 1:iMax
        ...
        A(ineqNum,delta(k+1)) = alpha(i, k+1);
        A(ineqNum,l(i,k+1)) = -1;
        b(ineqNum) = 0; %this line is technically not needed since we initialized b as zeros
        ineqNum = ineqNum + 1;
    end
end
